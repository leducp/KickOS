...
// backends share: everything below kos_uart_open. The two parts are on DIFFERENT ISAs, so
// they link no archive in common and this unit cannot live in one; uart_esp.cc enters each
// DRIVER's own archive instead, named by that driver's CMakeLists, and reads the configured
// chip's own <regs/uart.h>.
//
...
// commit protocol the LX6 has no counterpart for. The silicon contract below open is one
// contract; at open it is two.
...
{
    // TX-empty is armed on demand by kos_uart_write, so an open arms the RX sources alone.
    ...
    // Bounds every register poll in the family. Far longer than any legitimate wait, so a
    // dead channel degrades instead of hanging the caller.
