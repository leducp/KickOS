...
// the app's main. This TU compiles without -Dmain (it does not link kickos_core), so
// app.h's #ifdef main build-stamp block stays inert here.
...
extern "C" int kickos_default_init_run(int argc, char** argv)
{
    ...
    if (kos_send_timed(KOS_CAP_STDOUT, "", 0, kickos::driver::KOS_DRV_HANDOVER_PROBE_US) >= 0)
    {
        ...
        // endpoint served by ONE recv thread, which is every console driver in tree, the
        // second cannot be taken until that thread is back in kos_recv, which it reaches only
        // after the first flush returned, so this send completing IS the drained signal. The
        // proof holds for exactly that one-recv-thread shape: two of them could take both
        // probes concurrently.
