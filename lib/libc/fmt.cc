...
void emit_str(Sink& s, const char* str) {
...
extern "C" int kvsnprintf(char* buf, size_t size, const char* fmt, va_list ap) {
  Sink s{buf, size, 0};
  ...
  for (const char* p = fmt; *p != '\0'; p++) {
    ...
    switch (*p) {
      case 's': emit_str(s, va_arg(ap, const char*)); break;
      case 'c': s.put(static_cast<char>(va_arg(ap, int))); break;
      case '%': s.put('%'); break;
      case 'd':
      case 'i': {
        int64_t v;
...
extern "C" int ksnprintf(char* buf, size_t size, const char* fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
