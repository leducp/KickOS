...
// the service recorded (xmc_spi0_endpoint) and delegates a SIGNAL-narrowed copy, then
...
#include <kickos/driver/xmcssc.h>     // xmc_spi0_endpoint()
...
int main(int, char**)
{
    ...
    // config + endpoint + unprivileged driver) before this main. Grab the endpoint the
    // service recorded.
    int const ep = xmc_spi0_endpoint();
    if (ep < 0)
    {
