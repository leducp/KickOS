...
//
// KickOS M0 sim demo (unprivileged userspace, C++). Exercises every M0
// verification bullet across the syscall boundary:
...
//   - an MPU violation (wild write) is caught and reported via mprotect
...
void line(const char* s) { kos_write(1, s, strlen(s)); }
...
void wild_writer(void*) {
  ...
  char b[80];
  ksnprintf(b, sizeof(b), "[mpu] unprivileged write to guard %p (expect fault)\n",
            const_cast<int*>(g));
  line(b);
  *g = 0xdead;                        // faults: guard is PROT_NONE for user
  line("[mpu] ERROR: wild write did not fault\n");
  kos_sem_post(g_done);
}
