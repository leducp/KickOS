...
//
// K64F console handover demo: the kernel relinquishes the UART0 OpenSDA VCOM to an
// UNPRIVILEGED userspace driver (kickos_k64uart), and a normal worker's printf()
// output then reaches the wire THROUGH that driver, not the kernel chip path.
//
...
// per-AIPS-slot protection is the accepted K64F ceiling), so once the start shim opens
...
//
// Flow (privileged main):
//   0.   print ONE operator line via the still-kernel-owned console (before handover).
//   1-5. k64uart_console_start(): create endpoint E, kos_console_publish(E) (kernel
//        UART goes dark, stdout routes to E), open UART0's AIPS slot, spawn the
//        unprivileged driver granted the UART0 window + {E | WAIT}, then CLOSE root's
//        own WAIT cap on E (S4). driver_prio (12) >= worker prio (10) per D9.
//   6.   spawn a normal unprivileged worker that printf()s. Its libc _write self-sends
//        to cap index 0 (seated to E at spawn, because it is spawned AFTER publish),
//        rendezvous-delivered to the driver, poll-written to UART0.
...
namespace
{
...
#else
    // Scramble-then-panic reclaim test
    ...
    // AIPS slot the start shim opened, which IS the coarse-AIPS point being shown: a
...
int main(int, char**)
{
    // 0: one operator line while the kernel still owns the console. If nothing appears
    // on the wire after this line, the handover bring-up failed. Use kos::print (the
    // out-of-band kernel-console path), NOT stdout: cap index 0 is not seated until after
    // publish, so a stdout write here fails and pins newlib's process-global stdout probe
    // to dead, which then silences every post-publish console client (newlib_stubs D8).
    kos::print("k64console: handover starting; silence past this line on the wire means bring-up failed\n");

    // 1-5: publish + open the AIPS slot + spawn the driver + drop root's WAIT cap.
    if (k64uart_console_start(DRIVER_PRIO) != 0)
    {
        ...
        kos::print("[k64console] handover bring-up FAILED; not spawning worker\n");
        return 1;
    }

#if K64CONSOLE_SCRAMBLE_TEST
    // Scramble-then-panic test: handover is live (USER_OWNED, driver up). Allocate an
    // SRAM cell that we do NOT grant to the scrambler; its write there faults (the
    // mpu_fault mechanism). Give the driver a beat to emit its first-light banner, then
    // spawn the scrambler (NO MMIO grant, NO memory grant) that garbles UART0 and faults.
    // Scrambler prio > driver so it runs promptly.
...
#else
    // 6: spawn the printing worker AFTER a confirmed driver (so its index-0 cap is
    // seated to the published endpoint by cap_install_defaults).
