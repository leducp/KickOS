...
// controllers cannot honour them. Three of them are absences, so a comment is the only place
...
//      there is a count, and it is not optional.
...
extern "C"
{
    ...
    // DIFFERENCE ON. bus.h names kos_bus_rsp.len "rx bytes following"; on an error return
    // it is instead "bytes actually transferred before the NACK". A write-only transaction
    // makes the two disagree outright (three bytes transferred and no rx byte to follow),
    // so the sign of `status` is the discriminator:
    //
