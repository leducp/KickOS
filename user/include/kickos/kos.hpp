...
inline long write(int fd, const void* buf, size_t len) { return kos_write(fd, buf, len); }
inline long puts(const char* s) { return kos_puts(s); }
...
class Semaphore {
 public:
  explicit Semaphore(int initial = 0) : id_(kos_sem_create(initial)) {}
...
inline int spawn(void (*entry)(void*), void* arg, const char* name,
