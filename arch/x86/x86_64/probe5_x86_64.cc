...
//
// Privileged address-space tests without kernel threads.
// Fault-handler state is volatile because deliberate faulting instructions
// are not compiler-visible calls. Fault address and vector are 64-bit,
// beyond this project's Atomic size limit.
...
// Keep this marker in sync with tools/run-qemu-x86_64-x5.sh.
#define KICKOS_X5_TOKEN "KICKOS-X5 6b1e04c7 x86_64/q35 aspace"

// Hidden visibility keeps the reference PC-relative. Apply it to the first declaration.
...
namespace
{
    ...
    // The assembly asserts this instruction length for fault recovery.
    constexpr unsigned load_stub_bytes = 3;

    constexpr size_t granule = 4096;

    // Frames come from identity-mapped conventional memory.
    ...
    unsigned g_allocs = 0;
    // Zero disables failure injection; N rejects allocation N and all later ones.
    ...
    // Memory outside the pool must survive address-space destruction.
    ...
    // Fixed width for the run script's report pattern.
    ...
    // Descriptor fields checked by the tests.
    ...
    // Synthetic PAT values for decoding tests; never write these to IA32_PAT.
    // Power-up layout: WB at 0, WT at 1, UC- at 2, UC at 3 (Table 14-12).
    ...
    // Return leaf attribute bits, or a sentinel using execute-disable bit 63.
    ...
    // First unsupported physical address. Bits 51:MAXPHYADDR are reserved.
    ...
    // Write through the physical address; later reads use the per-space mapping.
    ...
    // Return whether a deliberate unmapped load faulted and resumed.
    ...
    // Return the mapped leaf's PAT index bits, or memtype_refused.
    ...
    void arm_boot_space(void)
    {
        ...
        // Use the unchecked walk to inspect kernel mappings in the boot space.
    ...
    // Create both spaces before editing to detect copied child tables.
    void arm_kernel_half_shared(void)
    {
        ...
        // Require the firmware layout used by the kernel-window placement test.
        ...
        arm("map_at_the_window_refused",
            do_map(g_space_a, kwin, g_frame_a, 1, ARCH_MAP_R, ARCH_MAP_NORMAL)
                == ARCH_ASPACE_EINVAL);
        // Check the kernel mapping and the user API's refusal of that address.
        ...
        // Read both spaces without modifying their tables.
        ...
        // Check through hardware translation too.
        ...
        // Edit under A and read under B to check shared table updates.
    ...
    // Check three root values and three translations of the same virtual address.
    void arm_root_switch(void)
    {
        ...
        // The switching code must be mapped at the same address in every root.
    ...
    void arm_refusals(void)
    {
        ...
        arm("refuse_write_and_execute",
            do_map(g_space_a, g_va_unmapped, g_frame_a, 1, ARCH_MAP_R | ARCH_MAP_W | ARCH_MAP_X,
                   ARCH_MAP_NORMAL)
                == ARCH_ASPACE_EINVAL);
        // x86 cannot express no-read or execute-only permissions.
        arm("refuse_no_read",
        ...
        arm("refuse_unknown_memtype",
            do_map(g_space_a, g_va_unmapped, g_frame_a, 1, ARCH_MAP_R,
                ...
                == ARCH_ASPACE_EINVAL);
        // A present boot-root slot belongs to the shared kernel mapping.
        arm("refuse_kernel_half_address",
            do_map(g_space_a, g_image_probe & ~static_cast<uintptr_t>(granule - 1), g_frame_a, 1,
                ...
                == ARCH_ASPACE_EINVAL);
        // Check the end of the physical range against hardware and descriptor limits.
        ...
        // Partial remaps must fail without changing existing leaves.
        arm("map_refuses_a_partially_mapped_range",
            do_map(g_space_a, g_va, g_frame_a, 2, ARCH_MAP_R, ARCH_MAP_NORMAL)
                == ARCH_ASPACE_EINVAL);
        arm("the_partially_mapped_range_kept_its_leaf",
            arch_aspace_frame_at(g_space_a, g_va) == g_frame_a
                and arch_aspace_frame_at(g_space_a, g_va_unmapped) == 0);
        // A wholly mapped range must still support remapping.
        arm("map_admits_a_wholly_mapped_range",
            do_map(g_space_a, g_va_unmapped, g_frame_a, 2, ARCH_MAP_R, ARCH_MAP_NORMAL)
                    == ARCH_ASPACE_OK
                and do_map(g_space_a, g_va_unmapped, g_frame_b, 2, ARCH_MAP_R, ARCH_MAP_NORMAL)
                        == ARCH_ASPACE_OK);
        arm("the_remap_of_a_wholly_mapped_range_landed",
            arch_aspace_frame_at(g_space_a, g_va_unmapped) == g_frame_b);
        arm("the_remapped_range_clears",
            do_unmap(g_space_a, g_va_unmapped, 2) == ARCH_ASPACE_OK
                and arch_aspace_frame_at(g_space_a, g_va_unmapped) == 0);
    ...
    {
        // Decode the leaf against the live PAT.
        ...
        // Use synthetic PATs to test other layouts without retyping live mappings.
        ...
        // Compare actual leaf bits with the decoder result. This firmware uses the
        // power-up PAT, so this check alone cannot detect a hard-coded default.
    ...
    void arm_span(void)
    {
        // Cross two leaf tables with a 600-page mapping.
    ...
    {
        // Measure allocation cost so failure injection reaches the last table.
        ...
        // Fail before any leaf is installed.
        ...
        // Fail after some leaves were installed to exercise rollback and pruning.
        c = arch_aspace_create();
        ...
        // Keep a live page in a neighboring leaf table. Rollback must prune empty
        // tables without freeing the shared parent or the live page's table.
        c = arch_aspace_create();
        ...
        // Unmap borrowed frames before destroying the space.
    ...
    void arm_destroy(void)
    {
        ...
        arch_aspace_activate(g_boot);
        // Unmap all borrowed test frames before destroying either space.
        ...
        // Find a shared slot whose accessed bit differs between roots. Destruction
        // must recognize shared ownership despite the differing descriptors.
        ...
        // An owned frame left mapped must be freed with the space.
        unsigned const held = g_outstanding;
    ...
    void arm_frames_returned(unsigned baseline)
    {
        ...
        put_dec(g_allocs);
        put("\n");
        // All owned frames and tables must be reclaimed.
...
// Ignore frames this pool did not allocate.
...
// Kernel hooks for an image without threads. Only the fault reporter
// handles events, allowing deliberate faults to resume.
...
void kickos_x86_64_landed(uintptr_t ram_base, uint64_t ram_size)
{
    ...
    // Reserve test frames before failure injection can affect allocation.
