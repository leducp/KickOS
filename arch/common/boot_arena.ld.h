...
/* Link-time replay of the first two arch_ram_alloc() calls kmain makes -- the idle
 * boot stack, then the root boot stack -- so an arena that cannot hold them fails the
 * BUILD instead of kpanicking on hardware. The alignment run-up is the whole point: on
 * a pow2-descriptor arch each block's base is snapped to its own (rounded) size, so a
 * fit computed as ram_start + idle + root is optimistic by up to two padding runs.
 *
 * KICKOS_BOOT_* arrive as -D from arch/CMakeLists.txt, which reads the sizes from the
 * board's board_config.h and the granule from arch_mpu_min_region()'s own source, so
 * this cannot model a geometry the allocator does not produce. The arena symbols are
 * arguments because the RX ABI spells them with an extra leading underscore.
 */
