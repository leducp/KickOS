...
// links unchanged into BOTH the kernel and an unprivileged userspace USIC driver.
// KickOS links kernel + driver into one MPU-partitioned ELF, so this leaf is one
// shared read-only copy and must carry no writable state.
//
...
namespace driver
{
    ...
    // of TCSR, so side-effect-free and safe on any clocked USIC channel.
