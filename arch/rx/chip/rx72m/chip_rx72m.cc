...
namespace
{
    ...
    // --- SYSTEM / low-power registers (UM §9, §11, §13) ---
    constexpr uintptr_t SYSTEM_PRCR = 0x000803FE;   // 16-bit protect register (§13)
    ...
    constexpr uint32_t MSTPA_CMTW1 = 1u << 0;       // UM §11 MSTPCRA b0 = CMTW unit 1
    constexpr uint32_t MSTPA_CMTW0 = 1u << 1;       // MSTPCRA b1 = CMTW unit 0
    constexpr uint32_t MSTPB_SCI6 = 1u << 25;       // MSTPCRB b25 = SCI6 (§11, MSTPB25)

    // --- ICU (for the CMTW0 timer line; UM §15) ---
    ...
    // --- Clock generation circuit (UM §9), all PRC0-protected ---
    constexpr uintptr_t SYSTEM_SCKCR = 0x00080020;   // 32-bit system clock control (§9.2.1)
    constexpr uintptr_t SYSTEM_SCKCR3 = 0x00080026;  // 16-bit clock source select (§9.2.4)
    constexpr uintptr_t SYSTEM_PLLCR = 0x00080028;   // 16-bit PLL control (§9.2.5)
    constexpr uintptr_t SYSTEM_PLLCR2 = 0x0008002A;  // 8-bit PLL stop control (§9.2.6)
    constexpr uintptr_t SYSTEM_MOSCCR = 0x00080032;  // 8-bit main osc control (§9.2.8)
    constexpr uintptr_t SYSTEM_OSCOVFSR = 0x0008003C; // 8-bit stabilization flags (§9.2.14)
    constexpr uintptr_t SYSTEM_MOSCWTCR = 0x000800A2; // 8-bit main osc wait control (§9.2.17)
    constexpr uintptr_t SYSTEM_MOFCR = 0x0008C293;   // 8-bit main osc forced osc/drive (§9.2.19)
    ...
    // MODRV2[1:0]=00 selects the 20.1..24 MHz crystal drive range (§9.2.19); the
    // board crystal is 24 MHz. MOFXIN=0 (no forced oscillation).
    constexpr uint8_t MOFCR_XTAL_24MHZ = 0x00;
    // MSTS wait count off the LOCO: MSTS > (tMAINOSC * fLOCO_max + 16)/32 (§9.2.17).
    ...
    constexpr uint16_t SCKCR3_CKSEL_PLL = 0x0400; // CKSEL[2:0]=100 => PLL (§9.2.4)
    ...
    // --- MPC (UM §23): pin-function mux for the SCI6 console pins ---
    constexpr uintptr_t MPC_PWPR = 0x0008C11F;    // write-protect (§23.2.1)
    constexpr uint8_t PWPR_PFSWE = 1u << 6;       // PFS register write enable
    constexpr uint8_t PWPR_B0WI = 1u << 7;        // PFSWE write disable
    constexpr uintptr_t MPC_PB0PFS = 0x0008C198;  // PB0 pin function (§23.2)
    constexpr uintptr_t MPC_PB1PFS = 0x0008C199;  // PB1 pin function
    constexpr uint8_t PFS_PSEL_SCI6 = 0x0B;       // PSEL=001011b: PB0->RXD6, PB1->TXD6 (§23)
    ...
    // Actual baud 60e6/(8*65) = 115385 (+0.16% error). (UM §42 async baud table.)
    ...
    bool clock_to_pll_240mhz()
    {
        ...
        uint8_t mosccr_rb = r8(SYSTEM_MOSCCR);  // read back before dependent writes (§9.2.8)
        (void)mosccr_rb;
    ...
    {
        // Pin mux: route PB1->TXD6, PB0->RXD6 (UM §23). Unlock the MPC (clear
        ...
        for (volatile uint32_t d = 0; d < 10000u; d++) // >= 1-bit settle before TE (§42 init)
        {
        }
...
void arch_init(void)
{
    ...
    // Release the module stops for the timer + console (UM §11 MSTPCR).
