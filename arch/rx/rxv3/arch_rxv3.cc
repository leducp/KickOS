...
// switch.S hard-codes each offset below as a literal displacement taken from
// rx_trap_stack.h. A field inserted ahead of the bounds leaves a USP guard comparing
// against trace_tid, and that still assembles and still links: these assertions are the
...
// With no block seated every syscall takes svc_trampoline's refusal instead of dispatching,
// so this arch cannot be configured without the blocks. ARCH_KERNEL_STACKS_MANDATORY puts
// `range 1 1` on the knob; this fires if that select is ever dropped.
static_assert(KICKOS_KERNEL_STACKS != 0,
              "rxv3's syscall trap runs every dispatch on ctx.kernel_sp");

// The syscall guard runs above the arm selection, so its one figure must dominate both
// arms. The generic arm sets it; the fastpath arm's save plus restore descent is smaller.
...
// accepted over svc_trampoline before it reaches the block builds its save on the same USP
// below that head (rx_trap_stack.h derives the composition).
...
// block top, plus the switcher zone a preemption takes below the deepest dispatch byte.
...
// svc_trampoline moves R0 to ctx.kernel_sp before it calls anything, so the generic arm's
...
// not, one of the two classes is wrong.
...
//
// NEST_EXIT is a plain integer because the gate scrapes it as an immediate, so nothing in
// the language ties it to its terms: the 8 bytes arch_fault_redirect_to_exit skips below the
// block top so kickos_rx_pendsw's block leg does not read the USP as zero distance, plus one
// whole switcher zone below the descent.
...
// arch_irq_save raises PSW.IPL with an MVTIPL immediate, so the literal in the asm string
// and this constant must agree.
...
namespace
{
    ...
    // CMTW1 is 32-bit, extended to 64 in software by catching wraps on each read, so a wrap
    // not observed within one 2^32-cycle period is MISSED. The two words are one value,
    // kept coherent by the IrqLock in now_cycles.
    uint32_t g_cyc_high = 0;
    uint32_t g_cyc_last = 0;

    // Bumped only by the first-level DEVICE-IRQ dispatchers, never by the syscall INT path,
    // so arch_in_isr() reads false throughout syscall_dispatch (arch.h contract).
    Atomic<int, Order::RELAXED> g_in_isr = 0;

    // Software IRQ controller for INJECTED logical lines: the ICU's two software interrupts
    // are the only ones settable from software, so every injected line rides the single
    // SWINT2 doorbell. Lines < SOFT_IRQ_LINES are software; lines >= it are real ICU vectors
    // gated by ICU.IER. RX's own sub-32 vectors (SWINT 26/27, timer CMWI0 30) are configured
    // directly by the arch/chip init and never pass through the arch_irq_* seam.
    constexpr int SOFT_IRQ_LINES = 32;
    uint32_t g_irq_masked = 0xFFFFFFFFu;
    Atomic<int, Order::RELAXED> g_inject_line = -1;
    // bit set = a raise landed on this soft line while masked, latched one-deep and
    // redelivered through the SWINT2 doorbell at unmask.
...
#define KICKOS_RX_MPU_TRACE 0
    ...
    void rx_mpu_mark(char c)
    {
        ...
        while ((reg8(SCI6_SSR) & (1u << 7)) == 0)
        {
            if (++spin > 200000u)
            {
                return; // a wedged FIFO must not block the localizer itself
            }
        }
...
extern "C"
{
    // A user thread whose entry returns must trap out via the exit syscall: it cannot run
...
namespace
{
    ...
    {
        // The wrap-extend read must be atomic against a concurrent reader.
    ...
    inline void icu_ier_set(int line, bool on)
    {
        ...
        // IER packs 8 lines per byte, so this RMW must be atomic against a device ISR's
        // arch_irq_mask on a SIBLING line. Reachable from unlocked syscall context.
        arch_irq_state_t s = arch_irq_save();
        if (on)
        {
    ...
    // IR (UM sec.15.2.1) and IER (sec.15.2.2) are indexed 1:1 by vector; IPR (sec.15.2.4)
    // is SHARED, several sources collapsing onto one entry, so the IPR index is not the
    // vector number in general. An unlisted vector falls back to identity, which holds for
    // the 1:1 SCIg/peripheral block this backend arms. A shared source reaching identity
    // gets the wrong IPR: list it here, citing the UM, before enabling that device line.
...
// Identical to what the SWINT switcher saves, so the first switch-in (arch_start) restores
// it and RTEs into entry(arg). Low to high on the USP:
                       ...
                       int privileged)
{
    ...
    *(--sp) = 0;                                 // A0 low
    ...
    // control words take the DPFPU reset posture per the RXv3 ISA UM. The layout must
    // mirror DPUSHM.D dr0-dr15 then DPUSHM.L dpsw-decnt: DR block first, then DECNT, DCMR,
    // DPSW with DPSW lowest = the new ctx.sp.
    ...
    *(--sp) = 0x00000100u;                       // DPSW (lowest word)
    ...
    // store a frame through it. `top` is the aligned high edge, so a running thread's USP
    // stays in [stack_lo, stack_hi].
                       ...
                       void* stack_base, size_t stack_size)
{
    ...
    // (arch_fault_redirect_to_exit derives it). Here the clearance comes only from the
...
arch_irq_state_t arch_irq_save(void)
{
    ...
    // MVTIPL takes an immediate only, so the level is a literal (the static_assert above
    // pins it), and it is self-synchronizing.
...
// --- Fault isolation --------------------------------------------------------
// The saved pair is [0]=PC, [4]=PSW on the ISP: exception pre-processing writes both to the
// stack ISP selects, never to the USP (RXv3 ISA UM sec.5.2 + Table 5.2), and RTE pops PC
// then PSW. `cause` rides along because the fixed-vector offset is known only to the shim
// that took the vector.
struct RxFaultFrame
{
    uint32_t* saved;
    uint32_t cause;
...
// The five instruction-CANCELING exceptions, whose saved PC is the instruction that
// generated them (ISA UM sec.5.3.1 Table 5.1). Cause 0, the _rx_trap catch-all, carries the
// NMI and BRK: an NMI is accepted at an instruction boundary with PSW.PM still set, so
// admitting it here would kill whichever thread happened to be running for a chip event.
...
bool arch_fault_is_user_thread(void* frame)
{
    ...
    // which no user thread can reach.
    ...
    // lie in the thread's own stack: a thread that wrecked R0 would otherwise put privileged
    // code on a stack of its choosing. Length 0 asks for containment alone, the stack
    // growing DOWN from here with no frame written at this address. The thread-stack test
    // and not its kernel-block twin, because a fault taken while a thread runs its dispatch
    // on ctx.kernel_sp arrives with PSW.PM clear and is refused above.
    uint32_t usp;
...
#if KICKOS_HAVE_MPU
    // A STACK OVERFLOW reads as in-bounds in the USP: every one of these exceptions CANCELS
    // its instruction and restores SP (ISA UM sec.5.3.1), so the denied push leaves the USP
    // where it was and the containment test above passes with nothing left below. MPDEA is
    // the only register that says so, and supervisor bypasses the RX MPU, so a stub run on
    // the exhausted stack smashes its way out untrapped.
...
void arch_fault_redirect_to_exit(void* frame)
{
    ...
    if (ff->cause == 0x54)
    {
        ...
        // Latched, and cleared only through MPECLR: a bit left set labels the NEXT thread's
        // fault with it.
    ...
    // The same posture the syscall trap's own rewrite builds (switch.S
    // kickos_rx_syscall_trap): supervisor on the USP. U is WRITTEN rather than inherited
    // because RTE forces it only on a transition TO user mode and this one is the other way.
    // I=1 with IPL=0 is what lets the stub's exit_current reschedule: it pends SWINT and
    // must see it taken.
...
// ON THE ISP, .Lsvc_nokstack having cleared PSW.U before the branch; `usp` is the thread's
// own, copied before that clear.
...
// C side of the RX exception handler (startup.S .fvectors shims branch here with r1=cause,
// the fixed-vector offset, and r2=the saved PC/PSW pair on the ISP). RETURNING means the
// fault was redirected: the shim's RTE then lands in kickos_thread_fault_exit instead of
// resuming the faulting instruction.
void kickos_rx_fault_report(uint32_t cause, uint32_t* saved)
{
    // Nothing may run above this, not even the raw localizer below: kpanic_enter's console
    // reclaim is permanent and this fault is survivable.
    ...
    // thread hitting an ungranted region. MPESTS.DMPER is an operand access (address in
    // MPDEA, DRW gives read vs write), IMPER an instruction fetch (address is the stacked
    // PC). From supervisor it cannot be an MPU fault, so it falls to the generic dump.
...
// The free-running 32-bit CMTW1 counter, read raw: it wraps on its own and the host
// reconstructs absolute time from the SESSION clock_hz anchors (arch.h). Same source as
// arch_clock_now.
...
void arch_timer_arm(uint64_t deadline_ns)
{
    ...
    // Clamp to the 32-bit one-shot range BEFORE converting, so a far-future deadline cannot
    // overflow ns*freq. A clamped deadline fires early and the kernel re-arms the remainder.
    uint64_t f = kickos_rx_timer_hz;
    ...
    // The gate that actually arms the CMWCOR compare: without it the counter free-runs past
...
// From the SWINT switcher (kickos_rx_pendsw to kickos_rx_restore, and the arch_start
// first-entry path) after the physical swap. That handler already runs with PSW.I=0; the
...
void kickos_arch_mpu_commit(void)
{
    ...
    // Overlaps OR their permission bits with the background (UM sec.17.1.4), so MPBAC must
    // stay 0 or every user thread is silently granted. MPOPI.INV clears any stale region V
    // bits. MPEN takes effect on the RTE into user mode (UM sec.17.2.3).
...
// An RX MPU region pair encodes permissions only, and every RX part in tree runs the arena
// uncached, so a nocache request is already satisfied.
int arch_mpu_nocache_support(void)
{
    return ARCH_MPU_NOCACHE_ALREADY;
}

// Rule 7 (arch.h): bit-band is a Cortex-M alias window, and an RX link resolves this seam
// here, so the definition is mandatory.
...
void arch_irq_unmask(int line)
{
    ...
    {
        g_irq_masked &= ~(1u << line);
        // A raise taken while the line was masked redelivers now through SWINT2, on the
        // normal ISR path rather than as a direct post.
    ...
    {
        // Priority BELOW the kernel lock level before enabling, so a device line cannot
        // preempt an IrqLock-held section. IPR is shared per the ICU source table, so the
        // index comes from vector_to_ipr and NOT the vector; IR/IER stay vector-indexed.
        // IR latches a request while IER=0, so re-enabling here fires a raise taken while
        // masked.
        reg8(ICU_IPR_BASE + vector_to_ipr(line)) = static_cast<uint8_t>(IPL_DEVICE);
        icu_ier_set(line, true);
    }
...
void arch_irq_clear_pending(int line)
{
    ...
    {
        // A group source is level-detected: GRPxxx.ISj and the group's IRn both FOLLOW the
...
// GCC emits the full GPR save and RTE for these. The INTB table (chip startup) routes CMWI0
// to the timer ISR and every other line to the default stub.
...
// SCI6 transmit-data-empty (TXI6, vector 87). TXI6 is edge-triggered (UM sec.15.3.1 p.523)
// and the ICU clears IR087 on accept, so no source flag is touched here; console_tx_isr
// gates SCR.TIE, which re-arms the edge.
//
// The INTB slot is fixed in flash, so this vector is SHARED between two owners over the life
// of the boot. Once kos_console_publish has run console_tx_deinit a raise here belongs to a
// driver that claimed vector 87, and routing it to console_tx_isr would find an empty ring
// and call the backend's irq_disable, clearing SCR.TIE behind the driver's back.
...
// SWINT2 doorbell: the software IRQ controller's delivery vector, pended by
// arch_irq_inject. IR026 is cleared FIRST because a software interrupt's request flag
// survives acceptance, and leaving it set re-fires the doorbell forever.
...
// The shared first-level ISR for every INTB device slot with no dedicated one. RXv3 leaves
// the accepted vector unreadable from the handler, so the chip reads its OWN status
// registers and posts what it finds.
...
// SCI6 receive-data-full (RXI6, vector 86). A DEDICATED slot rather than the shared dispatch
// above because an edge source cannot be discovered after the fact: the ICU clears IR086 on
// acceptance (UM sec.15.2.1(1) p.480), so a dispatch finds the flag it must test already 0.
// No flag is written here for the same reason: a write after the handler discards a request
// latched while irq_event_isr had the line IER-masked, dropping a received byte.
...
void kickos_rxv3_init(void)
{
    ...
    // SWINT is the deferred-switch line, at the lowest active priority so a switch requested
    // from an ISR is accepted only after every other ISR drains, and below IPL_LOCK so an
    // IrqLock masks it and the switch fires as the lock releases. SWINT and SWINT2 share
    // this IPR (regs.h).
    ...
    // SWINT2, the software-inject doorbell, shares SWINT's IPR set above but needs its own
    // IER bit.
    icu_ier_set(SWINT2_VECTOR, true);

    // CMTW1: CCLR=001 disables clearing, so the 32-bit counter wraps at 2^32 on its own and
    // is read-extended in software. Also the raw telemetry trace clock (arch_trace_now).
