...
- **`user-args-validated-at-boundary`** -- Every user-supplied scalar must be range-checked or clamped at the syscall boundary before use, and an invalid one returns a negative -KOS_E* code (never panics): priority is bounds-checked before it indexes ready lists / shifts a 1u<<prio bitmap; console len is clamped to 4096; injected/attached IRQ lines are checked against [0,KICKOS_MAX_IRQ); an unknown syscall number returns -KOS_EINVAL. The MPU does NOT make this optional now that it enforces. `syscall_dispatch` runs PRIVILEGED, so it is on the wrong side of the very boundary the MPU checks: an unvalidated scalar becomes an out-of-bounds access by the KERNEL, which the region set permits by construction. Enforcement contains a bad *user* pointer; only these checks contain a bad user *index*.
  - *applies:* user/kernel boundary; kernel (all arches)
  - *source:* kernel/syscall/syscall.cc (KOS_SYS_KCONSOLE_WRITE len clamp; the KICKOS_MAX_IRQ checks on inject/unmask/attach; the dispatch `default:` -KOS_EINVAL); kernel/syscall/syscall_thread.cc (thread_spawn prio bounds vs KICKOS_PRIO_MIN/MAX)
...
- **`init-return-is-shutdown`** -- RETURNING from `kickos_init_entry` is a single-shot system shutdown with that int status: root_entry calls the `kos_shutdown` SYSCALL, which reaches `kickos_terminate` -- drain the buffered console (`console_tx_flush_sync`), then `arch_shutdown(status)`. A persistent init must NEVER return (it parks or loops) -- running off its end shuts the whole system down. The shutdown is a gated syscall, not an unconditional call: it needs `AUTH_SYSTEM`, so a root that has narrowed that bit away (`authority-cap-narrows-only`) is REFUSED `-KOS_EPERM`, and root_entry then panics `"root: shutdown refused"` rather than running on. An app whose main returns must therefore keep `AUTH_SYSTEM`; the failure is loud, which is why the bit is not forced back on.
  - *applies:* boot/init lifecycle; kernel
  - *source:* kernel/init/kmain.cc (root_entry's tail: kos_shutdown then KICKOS_UNREACHABLE); kernel/init/console.cc (kickos_terminate: console_tx_flush_sync then arch_shutdown); kernel/syscall/syscall.cc (the KOS_SYS_SHUTDOWN arm's AUTH_SYSTEM gate); system/include/kickos/sys/init.h
