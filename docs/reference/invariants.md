...
- **`own-create-skips-reserved-cap-index`** -- The well-known cap indices [0 .. KICKOS_CAP_FIRST_DYNAMIC) (today [0..4); index 0 = KOS_CAP_STDOUT) are FROZEN reserved slots (`user/include/kickos/sys/cap_index.h`). An own-create (sem/mutex/endpoint create) scans placement from KICKOS_CAP_FIRST_DYNAMIC (cap_install), so it can NEVER land on -- and alias -- a reserved index. A reserved slot is written ONLY by the kernel (cap_install_defaults seats stdout and is the sole writer of index 0) or by explicit spawn delegation (delegated cap i lands at index i+1, filling 1..3); cap_install_at rejects index 0 outright. Never renumber a frozen entry; append by raising the last reserved index and KICKOS_CAP_FIRST_DYNAMIC together, the cap.h static_assert guaranteeing >=1 dynamic slot survives.
  - *applies:* user/kernel boundary, capability naming; kernel (all arches)
  - *source:* user/include/kickos/sys/cap_index.h; kernel/syscall/cap.cc (cap_install scan from KICKOS_CAP_FIRST_DYNAMIC:370-387, cap_install_at reject index 0:352-368, cap_install_defaults:431-450)
