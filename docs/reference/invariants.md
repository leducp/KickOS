...
- **`privilege-escalation-gated`** -- Privilege-crossing syscalls must be gated on the caller being privileged: spawning a privileged thread and irq_attach reject an unprivileged caller with -KOS_EPERM, and ram_alloc rejects with NULL (it is out of the -KOS_E* scheme, see `syscall-return-abi`). There is no path by which an unprivileged thread obtains privilege or the whole-arena grant.
...
- **`init-return-is-shutdown`** -- RETURNING from `kickos_init_entry` is a single-shot system shutdown with that int status: root_entry drains the buffered console (`console_tx_flush_sync`) then calls `arch_shutdown(status)`. A persistent init must NEVER return (it parks or loops) -- running off its end shuts the whole system down.
  - *applies:* boot/init lifecycle; kernel
  - *source:* kernel/init/kmain.cc (root_entry's tail: console_tx_flush_sync then arch_shutdown); system/include/kickos/sys/init.h
