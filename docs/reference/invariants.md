...
- **`ctors-run-before-init-entry`** -- App / libstdc++ / library global constructors run in the kernel root thread (kernel live, thread context so a ctor may issue a syscall like kos_clock_now) BEFORE `kickos_init_entry` is entered -- the normal C++ order. A custom init must not assume its own bring-up precedes those ctors. On sim / not-yet-migrated chips the `__kickos_app_init_array_*` symbols are null (weak) and the ctors instead run via the host runtime / the chip's own `.init_array` loop.
  - *applies:* boot/init ordering, C++ ctors; kernel + migrated chips
  - *source:* kernel/init/kmain.cc:36-45,114-125
