...
# The kernel console (KickOS Book)

> Book chapter (tier 1 core mechanism + tier 3 per-chip peripheral). Describes how
> the console works *as built*, and the invariants a change must not break — not a
> code paraphrase. Draft; to be refined after M3. Supersedes the throwaway
> `console-design.md` (pre-implementation design) and `console-buffer-spike.md`
> (the 11i spike), both pending removal in the M1.x squash.
...
[architecture.md](architecture.md) 12b, "Console device handover"). Keep that
...
The ring decouples a write from the UART bit rate. Before 11i, `arch_console_write`
was a polled busy-wait on the TX-ready flag — telemetry measured it as the single
largest on-CPU cost (~879 µs per burst at the K64F FEI clock). Now the producer
copies and returns; the bytes leave later, in an interrupt.
...
sizing (512 B, > `kprintf`'s 256 B buffer) keeps this path rare.
...
ring never names a register; the chip supplies only these pokes.
...
after `irq_init`; steady-state logging from then on is buffered. So 11i speeds
...
4. **Panic output must not depend on the buffered path or on a debug probe.** It
   ...
   owns the UART (12b) — must *reclaim + reinit* the peripheral and polled-print,
...
## Future (12b, capability handover)
...
12b, "Console device handover" and "Panic transport must be probe-independent."
