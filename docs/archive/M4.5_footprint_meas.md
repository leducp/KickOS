...
# M4.5 flash and RAM footprint measurement capture

**A capture, not a current fact.** Every number below describes a binary that was linked on
2026-07-29. It is not the tree's present footprint and must not be quoted as one.

**Provenance.** The fourteen-board sweep was produced on branch tip `ae7996e`
(`M4.5.2-unpriv-root-stage2`) with the Arm GNU Toolchain 15.3.1, GCC 15.3.1 for RX and Xtensa, by
building all fourteen real boards in both `KICKOS_ENABLE_SELFTEST` settings. Figures marked
`MinSizeRel` were re-measured at `176109e`; they agree with the explicit-`-Os` figures taken at
`ae7996e` to within 8 bytes, the tree having moved between the two. The erosion baseline is
`181540e` (the M4.5.1 merge). Every `-O0` figure is retained as the evidence for the build-type
change and describes no configuration the tree still produces.

**Which kind of unreproducible this is.** The weaker kind. These figures can be re-measured today:
the method is written down in section 2 and the toolchain is the one in `.session/env.sh`. What a
rebuild yields is a DIFFERENT set of numbers, because the tree has moved, so re-running does not
recover this capture. That is unlike the silicon captures in `M1_raw_meas.md` and `M3_raw_meas.md`,
which are permanently unreproducible: the Due unit is retired, no real STM32F103C8 exists, and no
micro:bit unit is recorded. Archived rather than deleted because a measurement is not regenerable
prose, and because these are the figures several conclusions elsewhere in the docs rest on.

**Three sections measure configurations that no longer exist**, so reproducing them means
resurrecting dead code: section 5 (the two-board `-Os` block, deleted), section 8 (the
`boards/bluepill-c8` `stm32f103.ld` override, deleted), section 10 (`CMAKE_BUILD_TYPE=Debug`,
retired fleet-wide).

**The rulings this capture supports live in `../design-flash-footprint.md`**, which is the decision
record. This file owns the numbers and schedules nothing. `../../TODO.md` owns the work items and
the re-witness campaign.
...
discount.** `boards/bluepill-c8/configs/base/defconfig` sets `KICKOS_MAX_THREADS 2`,
...
(`CMakeLists.txt:55-57`), and `boards/f302nucleo/configs/base/defconfig` provides the
