...
# M3 endurance soak -- STRESS_FOREVER under enforcement (2026-07-22)

The deferred-commit MPU seam soaked overnight on all four enforcing backends via the
`STRESS_FOREVER` harness knob (folded into commit 0b9af4f), each board running the selftest
stress body in an unbounded loop under `mpu enforce`. Each iteration performs 600 naps,
**18000 thread handoffs**, and 2040 cap churns. Host-side capture: one `cat` per board VCOM
into `.session/logs/soak-<board>.log`.

Final snapshot (2026-07-22, captures then stopped):

| board | arch / MPU        | iters   | handoffs/iter | enforced switches |
|-------|-------------------|---------|---------------|-------------------|
| rx72m | RXv3 / RX-MPU     | 79637   | 18000         | ~1.433 B          |
| xmc   | armv7m / PMSAv7   | 33885   | 18000         | ~0.610 B          |
| k64f  | armv7m / SYSMPU   | 21704   | 18000         | ~0.391 B          |
| c6    | rv32imac / PMP    | 10525   | 18000         | ~0.189 B          |

**Total: ~2.62 billion enforced context switches, zero FAIL / halt / panic / fault** across
all four backends (scanned for `FAIL|halted|panic|fault|HardFault|MemManage|BusFault|UsageFault`,
all clean). The per-switch `kickos_arch_mpu_commit` seam holds under sustained enforcement on
SYSMPU, PMSAv7, RX-MPU, and PMP silicon.

## Teensy 4.1 / i.MX RT1062 (Cortex-M7, PMSAv7) -- FIFTH enforcing backend (post-fix)

The first M7 under MPU enforcement, once the ERR011573 speculative-access fix landed (fixed
region wrap of the unbacked FlexSPI/SEMC apertures + I-cache; `docs/design-teensy-mpu-hang.md`).
Selftest 43/43 under enforcement, then STRESS_FOREVER soaked with the I-cache config:

| board    | arch / MPU      | iters | handoffs/iter | enforced switches |
|----------|-----------------|-------|---------------|-------------------|
| teensy41 | armv7m / PMSAv7 | 1734+ | 18000         | ~31.2 M           |

Clean (zero FAIL/halt/fault). An M7 endurance run confirming the fix holds under sustained
enforced context switching (I-cache config). (Capture note: the host `cat` on the FTDI VCOM
can drop mid-run -- `soak-teensy-fix.log` stopped at 377 while the board ran on to 1734+ on
the live console.)

### D-cache validation (pre-M4)

With the L1 D-cache enabled (`teensy41-dcache-*`, `KICKOS_IMXRT_DCACHE=ON`; set/way invalidate
before enable): **selftest 43/43 under enforcement** on silicon (commit `eb21826`, banner
`mpu enforce`) -- the M7 D-cache is correct alongside the MPU fixed-region wrap and the
per-switch commit seam. Safe to validate now (single core, no DMA); DMA coherence is an M4+
concern.

STRESS_FOREVER soak with D-cache on, clean (zero FAIL/halt/fault):

| board          | arch / MPU      | iters | handoffs/iter | enforced switches |
|----------------|-----------------|-------|---------------|-------------------|
| teensy41 (D$)  | armv7m / PMSAv7 | 2113+ | 18000         | ~38.0 M           |

Visibly faster than the I-cache-only run -- ~100 iters / 45 s vs the earlier config -- which is
the point: D-cache is a real throughput win on the M7. This clears the pre-M4 default flip
(D-cache on by default) recorded in `TODO.md`.

# M3 branch fleet re-validation -- test -> bench -> soak under enforcement (2026-07-22)

The `m3-integration` branch (master..HEAD, ~20 commits: per-task capability handle table,
tier-1 IRQ latch-and-coalesce, wide-timer clock hardening, plus this session's D-cache /
ROM-cache / MPU-seam / banner work) touched shared kernel paths (`kernel/thread/thread.cc`,
`arch/arm/common/arch_arm_common.cc`, `kernel/bench/bench.cc`), so the whole fleet was
re-gated on silicon: selftest -> context-switch bench -> STRESS_FOREVER soak, per board,
UNDER MPU ENFORCEMENT (`-DKICKOS_HAVE_MPU=1`; the M3-correct config -- the plain `-st` preset
is `mpu off`). The gate is what surfaced the linker + ROM-cache items below.

## Real issues the gate caught (all fixed on-branch unless noted)

1. **Linker `.appbss` displacement -- runtime MPU fault on the enforce bench (FIXED, 5aeabb2 +
   assert b24d934).** An app with zero initialized globals has an empty `.appdata`; GNU ld drops
   an empty section's `ALIGN(_appdata_size)`, so `.appbss` (the app globals, e.g. bench `g_rounds`)
   landed ~128 KiB BELOW the aligned app-data MPU window -> every app-global access = DACCVIOL.
   Surfaced as `MPU FAULT` on the Teensy (`@0x20205964`) and RX72M (`bench_b @0x5b5c`) enforce
   benches; selftest passed throughout because it has a non-empty `.appdata` so the ALIGN sticks.
   Fix: pin `.appbss` to `__kickos_appdata_start + SIZEOF(.appdata)` (the form the RISC-V scripts
   already used -> C6/PMP never faulted). Hit the pow2-window boards (Teensy/RX/XMC/RP2040/F411);
   K64F immune (`ALIGN(32)`). Added a lower-bound link `ASSERT` so a recurrence fails loud at link.
2. **RX72M 8 KB flash ROM cache enabled (5ab2575)** -- ~3.0x enforce-bench win (see table).
3. **Teensy M7 L1 D-cache default ON (c050fc2)** -- validated above.
4. **Banner: app-line TZ offset (8a6ab70), made reconfigure-safe (9e6d466); flash-picotool `-t elf` (55e0d44).**

## Enforce fleet -- selftest / bench / soak (silicon, `mpu enforce`)

Selftest is 43/43 on every MPU board under enforcement (2 more than the 41 non-enforce: the
MPU-specific `mpu_privileged_guard` + `endpoint_bound` compile in). Soak = STRESS_FOREVER,
18000 handoffs/iter, zero FAIL/halt/panic/fault. Benches are post-linker-fix (>= 5aeabb2).

| board    | arch / MPU        | test  | bench throughput   | per-switch      | soak iters (final) |
|----------|-------------------|-------|--------------------|-----------------|--------------------|
| teensy41 | armv7m / PMSAv7 D$ | 43/43 | 81061 ctx-sw/s     | 83 cyc / 209 ns | 1635+  (~29 M)     |
| rx72m    | RXv3 / RX-MPU     | 43/43 | 64911 ctx-sw/s     | 64/95 cyc       | 389                |
| xmc4800  | armv7m / PMSAv7   | 43/43 | 25890 ctx-sw/s     | 75 cyc / 520 ns | 163                |
| esp32c6  | rv32imac / PMP    | 43/43 | 32803 ctx-sw/s     | 31 cyc          | 27                 |
| frdmk64f | armv7m / SYSMPU   | 43/43 | 17976 ctx-sw/s     | 68 cyc / 566 ns | 103                |
| f411disco| armv7m / PMSAv7   | 43/43 | 14992 ctx-sw/s     | 67 cyc / 797 ns | 95                 |
| picopi   | armv6m / PMSAv7   | 43/43 | 6300-7500 ctx-sw/s | no DWT (thruput)| 191+               |

**RX72M ROM-cache win, enforce-vs-enforce:** prior ledger ~46772 ns/sw / ~21379 ctx-sw/s ->
now 15405 ns/sw / 64911 ctx-sw/s = ~3.0x on both, the flash-instruction-fetch acceleration.

Every banner verified `mpu enforce` + the gate commit. No MPU fault on any enforce bench after 5aeabb2.

## Non-enforce boards

| board       | arch / MPU        | test  | bench          | soak                         |
|-------------|-------------------|-------|----------------|------------------------------|
| esp32-wroom | LX6 / none        | 33/33 | 55348 ctx-sw/s | pre-existing hang (see open) |
| pizero2350  | armv7m-M33 / none | 41/41 | pending        | pending                      |

WROOM `mpu off` is correct (no MPU). RP2350 (Cortex-M33) is `mpu off` pending the ARMv8-M PMSAv8
backend (`docs/design-rp2350-mpu-armv8m.md`); non-enforce selftest 41/41 confirms the M3 kernel
on the M33 core.

## In-env (QEMU / host), same HEAD -- all green

- qemu (mps2-an386, armv7m): 8/8 (selftest, sched_exit, fault_dump, cxxtest, fp_switch, oot_export)
- microbit (nRF51, armv6m -- the ARM MPU-seam path): 5/5
- qemu-riscv (virt, RV32IMAC / PMP): 6/6 (incl. cxxtest under PMP)

## Open (do NOT gate the M3 merge)

- **WROOM Xtensa windowed-ABI soak hang -- PRE-EXISTING (master fails identically), Xtensa-only.**
  Wedges once ~4 concurrent threads run entry->run->EXIT with no intervening block (boundary is
  exactly 4 = 64 AR regs / 16-per-window) -> WindowBase/spill drift across the trampoline-entry +
  non-blocking-exit path in `arch/xtensa/lx6/switch.S`. Selftest (33/33) passes because its
  4-concurrent workers BLOCK before exit. Capping the stress app was disproven (masks a real bug)
  and NOT done. Dedicated Xtensa fix in progress; ticketed in `TODO.md`.
- **RP2350 ARMv8-M PMSAv8 MPU backend** -- implementation in progress per the design spike; until
  it lands RP2350 stays `mpu off`.
- **ESP32-C6 enforce bench ns-scaling** -- cycle counts correct, ns conversion ~8x high (a C6
  clock/counter-rate mismatch: `rdcycle` traps there so the bench samples an MMIO counter whose
  rate differs from `SystemCoreClock`); not a bench-arithmetic bug. C6 chip-clock item, not M3.
