...
silicon-proven on every MPU flavour KickOS drives -- SYSMPU (K64F), PMSAv7 (XMC4800, stm32f411,
and imxrt1062 on the M7), PMSAv6/v6-M (rp2040), PMSAv8 (rp2350), RISC-V PMP (esp32c6 +
qemu-virt), RX-MPU (rx72m): an unprivileged cross-domain
store faults and does NOT complete on real silicon. **No enforcement backend in the fleet is
unwitnessed any more**: `stm32f411` was the last one held on build-and-link validation, and it
was witnessed on `f411disco` silicon 2026-07-29. The security model (domains, per-thread
...
| RISC-V PMP (ESP32-C6) | **YES** by PMP (SRAM enforcement silicon-proven) | PMP discriminates per-thread -- SRAM enforcement DONE on silicon (18/18 + mpu_fault); a SEPARATE APM/PMS bus unit defaults deny-user and needs a one-time global open for any unprivileged PERIPHERAL access -- programmed at boot in `arch_init` (`apm_open_ree0`; mechanism in `docs/design-c6-driver.md`) |
...
RISC-V PMP (c6blink). f411spi remains a build-only STM32-family reference: the disco is on the bench
now, but the app itself is still unwitnessed (see its entry above).
