...
This doc above names `arch_mpu_apply` as the strong-override symbol throughout,
written before the deferred-MPU-commit seam landed fleet-wide
(`docs/design-mpu-commit-deferred.md`). The as-implemented backend
(`arch/arm/common/arch_arm_pmsav8.cc`) adapts to that seam and follows the K64F
precedent exactly:
...
So the "strong override, reuse the armv7m arch" decision holds verbatim; only the
overridden symbol name is `kickos_arch_mpu_commit` (the deferred-commit seam), not
`arch_mpu_apply` (the eager-apply symbol this doc predated). The plan-of-record
below reads the same with that substitution.
