...
> *KickOS Book — Tier 2 (per-ISA). A teaching chapter: how a small preemptive
...
§3 freezes the trampoline's continuation on this thread's stack and resumes it,
...
processor. Meet the questionnaire in §1, respect the two couplings (single-frame
...
(chips), `docs/porting.md` (the code-synced reference). This chapter explains the
