...
- The PMP NAPOT backend (`arch_mpu_apply`, `:245-305`; 8 entries; `arch_mpu_min_region`
  = 8; `arch_mpu_region_encodable`).
...
- **Does Hazard3 implement PMP? Yes [to-verify count: RP-008373 / Hazard3 config].**
  ...
  entries (`arch_mpu_apply` unrolls pmpaddr0..7, `:227-242`). If Hazard3-on-RP2350 exposes
  fewer than 8 PMP regions, `arch_mpu_apply` must clamp (it already only writes what it
