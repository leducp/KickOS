...
| Option | Verdict |
|---|---|
| Build `teensy41` with `KICKOS_IMXRT_DCACHE=OFF` for the USB console posture | **CHOSEN for stage 1.** The knob already exists and is already a documented posture; the cost is a measurable core-side slowdown on one board, and it makes the first working backend depend on nothing new |
