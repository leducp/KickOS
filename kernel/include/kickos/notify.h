...
// calling kos_notify is another. Nothing here is an IRQ concept: an object with no line
// attached is a complete notification.
//
// Rights: CAP_WAIT to bind, unbind and wait; CAP_SIGNAL to signal and to be installed as a
// signaller. No new authority bit: claiming a LINE still needs AUTH_IRQ, and everything
// reachable from a notification is reachable from a capability already held.
...
namespace kickos
{
    ...
    // EVERY "NONE" BELOW IS ZERO, AND THAT IS A FOOTPRINT RULE RATHER THAN A STYLE ONE. The
    // whole of `struct Kernel` is one statically-allocated object, and ONE non-zero default
    ...
    // entries are BINDING POOL INDICES BIASED BY ONE and never handles: the chain is walked
    // only under IrqLock in thread context, where a freed slot has already been unlinked.
    ...
    // to it. No LIVE handle biases to zero: that would take the all-ones handle, which
    // SlotPool's reserved index keeps unmintable.
    ...
    struct Notification
    {
        // Raised bits nothing has consumed. Survives the bound thread's death: that is the
        // preserve-for-the-next-server behaviour, with no transfer to perform.
        uint32_t pending = 0;
    ...
    // unbadged capability is the unconfined one, a badged copy reaches its own bit and no
    // other, and `src_cap` itself must be UNBADGED or a holder could re-reach the whole
    // object. 0 with the new capability in *out_cap, or -KOS_E*: EBADF, EINVAL (bit at or
    // above KCAP_BADGE_BITS), EALREADY (the source is already badged), EMFILE (the caller's
    ...
    // its own, so closing the last capability while a thread is bound does not free the
    // object. Returns nothing else: a waiter cannot learn its signallers' badges, so the
    // honest accept-all mask is an argument of the wait.
    int notify_bind(Thread* c, uint32_t cap_handle);

    // Release `c`'s binding and drop the reference it took. Unconsumed bits are LEFT pending
    // for the next server. 0, -KOS_EPERM (`c` is not the bound thread), or -KOS_EBADF.
    ...
    // chained signaller whose badge is in `mask` and which owes a rearm is rearmed: that is
    // the whole of "no forgot-to-ack deadlock". Consumed bits are cleared from pending and
    // written to *out_bits, and their signallers owe a rearm at the next wait.
    // 0, or -KOS_E*: EBADF, EPERM (no CAP_WAIT, or `c` is not the bound thread), EINVAL (an
    // empty mask, which could only ever block forever), ETIMEDOUT, ECANCELED.
    ...
    // LEAVE pending. Runs in exit_current's first locked block and NOT inside cap_teardown,
    // whose sweep releases IrqLock every KCAP_TEARDOWN_CHUNK slots: an object still naming a
    // thread across one of those gaps is a wake into a thread already in teardown.
    ...
    // holds IrqLock. No leak-don't-strand arm and none owed: a parked waiter is the BOUND
    // thread, whose bind holds a reference of its own, so refs cannot reach 0 under it.
