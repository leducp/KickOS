...
void wq_push_back(WaitQueue& q, Thread* t) {
  t->qnext = nullptr;
  t->qprev = q.tail;
  if (q.tail != nullptr) q.tail->qnext = t; else q.head = t;
  q.tail = t;
}

void wq_unlink(WaitQueue& q, Thread* t) {
  if (t->qprev != nullptr) t->qprev->qnext = t->qnext; else q.head = t->qnext;
  if (t->qnext != nullptr) t->qnext->qprev = t->qprev; else q.tail = t->qprev;
  t->qnext = nullptr;
  t->qprev = nullptr;
}
