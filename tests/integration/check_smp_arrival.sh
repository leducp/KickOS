...
#
# Gate on secondary-core arrival for a qemu-arm64 image built at more than one core. Two arms
# over the same ELF, both reading the chip's own release lines.
#
...
#   short machine   the emulator gets one core FEWER. PSCI CPU_ON answers INVALID_PARAMETERS
#                   for a core the machine does not carry, so the image must refuse by name at
#                   core n-1 and exit KICKOS_FATAL_STATUS.
#
...
#
# usage: check_smp_arrival.sh <elf> <expect-cores> <live-ere>
...
_usage="usage: check_smp_arrival.sh <elf> <expect-cores> <live-ere>"
elf="${1:?$_usage}"
want="${2:?$_usage}"
live="${3:?$_usage}"
...
# THE CHIP'S OWN WORDING (arch/arm64/chip/virt_arm64). Each is matched as a literal, and each
# passes require_literal first: an empty marker makes every absence-assertion below vacuous.
BAD_ENTRY="KickOS: qemu-arm64 secondary entry is not identity-linked: 0x"
BAD_CPU_ON="KickOS: qemu-arm64 PSCI CPU_ON refused core "
BAD_CPU_ON_TAIL=", status 0x"
NO_ARRIVAL="KickOS: qemu-arm64 secondary never reached its entry: core "
BANNER_HEAD="# smp: "
BANNER_TAIL=" core(s) online"
for _m in "$BAD_ENTRY" "$BAD_CPU_ON" "$BAD_CPU_ON_TAIL" "$NO_ARRIVAL" \
          "$BANNER_HEAD" "$BANNER_TAIL"; do
...
count_literal "$BAD_CPU_ON"
if [ "$KOS_COUNT" -ne 0 ]; then
    printf '%s\n' "$OUT" | grep -F -e "$BAD_CPU_ON"
    fail "PSCI refused a release on a machine that carries all $want cores"
fi
...
count_literal "$NO_ARRIVAL"
if [ "$KOS_COUNT" -ne 0 ]; then
    printf '%s\n' "$OUT" | grep -F -e "$NO_ARRIVAL"
...
count_literal "$BAD_CPU_ON$short_index$BAD_CPU_ON_TAIL"
if [ "$KOS_COUNT" -eq 0 ]; then
    printf '%s\n' "$OUT" | grep -F -e "KickOS: qemu-arm64"
    fail "no '$BAD_CPU_ON$short_index$BAD_CPU_ON_TAIL' refusal on a machine of $short cores.
