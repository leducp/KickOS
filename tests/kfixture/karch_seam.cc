...
//
// kpanic ends the PROCESS with a message matching tests/lib/panic.ere, so a kernel
// invariant enforced by KICKOS_ASSERT rather than a return code is gated by registering one
// executable per case with that regex, as the sim panic gates do.
...
extern "C"
{
    ...
    {
        printf("not ok - fixture: kickos_terminate(%d) ended the arm\n", status);
        exit(1);
    }
}
